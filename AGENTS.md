...
> **Open items**: `Docs/submission/SUBMISSION_CHECKLIST.md` §4
...
| **Core Loop Design** | **20%** | **Invest to Harvest to Upgrade to Observe Growth.** The economy must be the driving engine. |
...
## 3. Pacing and the Three-Stage Ramp
...
language-learning game and makes **no pedagogical claim**. The joke is taught wordlessly
across three stages, and each stage must do its job:
...
The cue is **visual, never audio**. Tiers are read by colour and symbol. Bottom = `der` = Blue `#3A86FF` ▲ · Middle = `die` = Pink `#FF006E` ● · Top = `das` = Purple `#8338EC` ■.
Items are labelled **English-first**; the player never needs to know German.

### The 90-second guideline is deliberately not applied

An earlier revision of this file demanded Shift 3 inside 90 seconds. **That instruction is
withdrawn.** This is a slow-burn narrative sim, paced closer to a walking sim than an arcade
loop: the Lübeck arrival establishes the thesis rather than delaying it, and compressing it
turns the pick loop into colour-matching with nothing at stake. `check-story.js` measures the
Aha at **167s against a 90s budget** and reports it as over budget *by design*.

The metric this game is accountable to is **time to first action**, and it is met: first
interactive scene at **10s**, WG buzzer at **18s**, no menu wall, no unskippable cutscene.

**Therefore, for every agent:**-**Do not** retune scene durations, `budget_s`, or Act One's length to make the 90s figure pass.
- **Do not** point `?quickstart=1` at `shift_3_test`. That was tried, it produced a worse
  experience, it was reverted, and six assertions in `build/verify.js` now pin it. Quickstart
  lands on `shift_1_teach`, where the mechanic is taught.
- **Do** preserve the 0.0 / 1.5 / 2.5 icon-delay ramp: the Aha only exists because stages 1
  and 2 earn it.

Full reasoning: `Docs/THE_MAKING_OF.md` §10.
...
1. **All recorded audio / voice acting.** There has never been any. Every sound is synthesised
   at runtime from oscillators, apart from one inlined music track: SFX (`src/audio/sfx.js`) plus pitched per-character talk-blips
...
**Rule of thumb: if you are unsure whether something exists in the build, cut the claim.**## 4. Special Award Targets ($15K Each)
...
2. **Most Satisfying Progression ($15K)**:
   -The €20 to €250 tuition goal curve where every one of the 5 shop upgrades (**E-Bike**€45, **Thermal Bag**€50, **Shelf Labels**€25, **Pocket Notepad**€20, **Shift Rota Cards**€35) has an immediate, unmistakable mechanical and visual impact. Use these exact names, the catalogue lives in `src/data/shop.js`.
...
3. **The gameplay video**: whatever script it ends up using must match the actual playable flow and must be shootable from the current build. No narration may claim voice acting, spoken German, or learning outcomes (see the removed claims section below).
...
3. Verify that the loop executes end-to-end:
   `Boot to Hub to Shift to Ride to Doorway to Debrief Receipt to Bike Shop to Next Shift to Win / Lose`.
...
1. **Mandatory Folder Audit Before Creation**:
   - **Never create new files, textures, geometry, or systems from scratch if they already exist in the repository.**-All agents MUST search existing project directories (`src/`, `vendor/`, `Docs/`, `Tools/`) and reuse existing assets, shaders, modules, and data models first.
