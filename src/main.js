...
class GameEngine {
  ...
  startFirstShift(startSceneId = 'shift_1_teach') {
    ...
    // pays once at shift_receipt. Difficulty comes from the scene's authored
    // unlocks.ramp.icon_delay_s (0 / 1.5 / 2.5), never from this counter.
    // Deriving it from the scene id made the quickstart receipt pay from shift
    // 3's wage table (19.00 base instead of 13.00) and label the opening frame
    // "WG Flat Party Surge", contradicting every published number.
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // ?quickstart=1 opens directly on Shift 1. This exists so the submission link
  // can land a reviewer in the core loop without putting a bypass button on the
  // main menu, where it would confuse a first-time player.
  //
  // It deliberately does NOT skip to shift_3_test. The Shift 3 Aha is that
  // purple means das, which the player learns across stages 1 and 2; dropped
  // cold onto the hardest stage a reviewer sees a rail pulse whose meaning was
  // never taught. P0.4 asks for Shift 3 to be REACHED inside 90 seconds, which
  // is a pacing job, not a teleport.
  try {
